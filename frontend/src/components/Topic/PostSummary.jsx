...
export default function PostSummary({ title, content, comments }) {
  ...
  // שימוש בהוק החדש במקום ה-useState המקומיים
  const { loading, error: genericError, rateLimitError, result: summary, summarize } = useGemini();
  ...
  const handleSummarize = async () => {
    ...
    const resText = await summarize({ title, content, comments });
    if (resText) {
      setOpen(true);
    }
  };
  ...
  return (
    <div className="mt-6">
      ...
      {rateLimitError && rateMsg && (
        <div className={`mt-4 flex items-start gap-3 rounded-xl p-4 border border-${rateMsg.color}-500/20 bg-${rateMsg.color}-500/5`}>
          {rateMsg.icon}
          <div>
      ...
      {summary && open && (
        <div className="mt-4 rounded-2xl border border-cyan-500/20 bg-slate-900/60 p-6 animate-fade-in">
