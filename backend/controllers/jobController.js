...
async function updateJob(req, res, next) {
  try {
    // ✅ מעביר את isAdmin ל-service
    const job = await jobService.updateJob(req.params.id, req.body, req.user.userId, req.user.isAdmin);
    res.json(job);
  } catch (err) { next(err); }
}
...
async function deleteJob(req, res, next) {
  try {
    // ✅ מעביר את isAdmin ל-service
    const result = await jobService.deleteJob(req.params.id, req.user.userId, req.user.isAdmin);
    res.json(result);
  } catch (err) { next(err); }
}
